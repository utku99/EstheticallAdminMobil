    ...
    useEffect(() => {
        ...
        Post("/api/Shared/ListCompanyOffices", {
            ...
            if (user?.userRoleId === 4) {
                const office = newOffices.find((item: any) => item.value === user?.companyOfficeId)
                setCompanyOffice([office])
            } else {
                setCompanyOffice([
                    { value: 0, label: "Ana Kurum" },
                    ...newOffices])
            }
        });

        Post(
            "/api/Shared/ListCompanyServices", {
            "companyId": user?.companyId,
            ...
            <HandleData data={[""]} loading={loading} title='Paylaşımınız Bulunmamaktadır'>
                ...
                <View style={{ width: SIZES.width * 0.9, flex: 1 }}>
                    ...
                    <CustomInputs
                        ...
                        dropdownData={services.find((item: any) => item.serviceId === formik.values.service?.value)?.subServices?.map((item: any) => ({ value: item.serviceSubId, label: item.serviceSubName }))}
                    ...
                    <View className=' relative'>
                        ...
                        <View className='flex-row items-center space-x-3 absolute right-0 top-4'>
                            <Text className='text-base font-medium text-customGray font-poppins'>{formik.values.statu ? "Aktif" : "Pasif"}</Text>
                            <Switch
                                thumbColor={"#FF8170"}
                                value={formik.values.statu}
                                onChange={() => formik.setFieldValue("statu", !formik.values.statu)}
                            />
                        </View>
                    </View>
                    ...
                    <CustomButtons type='solid' label='Paylaşım Yap' theme='big' onPress={formik.handleSubmit} />
